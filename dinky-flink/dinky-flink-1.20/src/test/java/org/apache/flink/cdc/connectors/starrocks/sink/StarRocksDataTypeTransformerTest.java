...
class StarRocksDataTypeTransformerTest {
    ...
    void mapsUnsignedBigintPrimaryKeyToLargeint() {
        StarRocksColumn column = convert(new DecimalType(false, 20, 0), true);

        assertThat(column.getDataType()).isEqualTo("LARGEINT");
        assertThat(column.isNullable()).isFalse();
    }

    @Test
    void keepsOrdinaryDecimalAsDecimal() {
        StarRocksColumn column = convert(new DecimalType(20, 0), false);

        assertThat(column.getDataType()).isEqualTo("DECIMAL");
        assertThat(column.getColumnSize()).contains(20);
        assertThat(column.getDecimalDigits()).contains(0);
    }

    @Test
    void keepsOtherDecimalPrimaryKeysAsVarchar() {
        StarRocksColumn column = convert(new DecimalType(false, 18, 2), true);
